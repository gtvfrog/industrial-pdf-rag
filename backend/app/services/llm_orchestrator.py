...
def build_rag_prompt(question: str, chunks: List[Chunk]) -> str:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        context_blocks.append(f"[{idx}] arquivo={filename}, página={page}\n    {text}")
...
RESPOSTA (em português, técnica, objetiva, com referências):
"""
    return prompt
...
class LocalLLMClient:
        ...
        if LocalLLMClient._model is None or LocalLLMClient._current_model_name != self.model_name:
            try:
                ...
                raise LLMProviderError(
                    ...
                    message=f"Failed to load local LLM: {str(exc)}",
    ...
    def answer(self, question: str, chunks: List[Chunk]) -> str:
        ...
        try:
            ...
            raise LLMProviderError(
                provider=self.provider_name,
                message=f"Local LLM inference failed: {str(exc)}",
                original_exception=exc,
            )
...
class GeminiLLMClient:
    ...
    def answer(self, question: str, chunks: List[Chunk]) -> str:
        ...
        try:
            ...
            raise LLMProviderError(
                provider=self.provider_name,
                message=f"Gemini API failed: {str(exc)}",
                original_exception=exc,
            )
...
def get_llm_client(settings: Settings, override_provider: Optional[str] = None) -> BaseLLMClient:
    ...
    if provider == "gemini":
        ...
        try:
            ...
            logger.warning(f"Failed to initialize GeminiLLMClient: {exc}; falling back to local")
            return LocalLLMClient(settings)
...
def answer_with_fallback(
    ...
    for provider in providers_to_try:
        try:
            ...
            logger.warning(f"Provider {provider} failed: {exc}. Trying next provider if available...")
            last_error = exc
            continue
